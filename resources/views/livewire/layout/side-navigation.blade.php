...
<div
    ...
    <ul class="flex flex-col mt-4 items-center space-y-4 text-center font-bold text-lg text-slate-300 ">
        ...
        <li class=" cursor-pointer w-[95%]  {{ $active === 'courses' ? 'side-active' : '' }}">
            <a wire:navigate href="{{  Auth()->user()->usertype == 1 ? route('courses.intro') : route('courses.index') }}" class="flex gap-1  hover:text-white"><svg
        ...
        <li class=" cursor-pointer w-[95%] {{ $active === 'grades' ? 'side-active' : '' }}">
            <a wire:navigate href="{{ route('grades.index') }}" class="flex gap-1 hover:text-white"><svg
                    xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor" class="h-6 w-6">
                    <path stroke-linecap="round" stroke-linejoin="round"
        ...
        <li class=" cursor-pointer {{ $active === 'resources' ? 'side-active' : '' }}"><a href="{{route('progresses.index')}}">progress</a></li>
